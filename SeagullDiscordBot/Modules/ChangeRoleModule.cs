...
namespace SeagullDiscordBot.Modules
{
	...
	public partial class ChangeRoleModule : InteractionModuleBase<SocketInteractionContext>
	{
		...
		[SlashCommand("change_user_role", "특정 사용자의 역할을 변경합니다.")]
		[RequireUserPermission(GuildPermission.Administrator)] // 관리자 권한이 있는 사용자만 사용 가능
		public async Task ChangeUserRoleCommand(
			[Summary(description: "역할을 변경할 사용자의 이름")] string username,
			[Summary("role", "변경할 역할의 이름")] string role)
		{
			...
			if (matchingUsers.Count == 1)
			{
				...
				var builder = new ComponentBuilder()
							.WithButton("역할 변경", $"change_role_confirm_button:{user.Id}:{role}", ButtonStyle.Danger);

				await RespondAsync($"'{user.DisplayName}({user.Username}, {user.Id})' 사용자의 역할을 '{role}'으로 변경하시겠습니까?", components: builder.Build(), ephemeral: true);

				// 로그 남기기
				Logger.Print($"'{Context.User.Username}'님이 '{user.Username}' 사용자의 역할 변경을 요청했습니다. 대상 역할: '{role}'");
			}
			// 사용자가 여러 명인 경우 선택 메뉴 표시
			else
			{
				var selectBuilder = new SelectMenuBuilder()
					.WithPlaceholder("역할 변경할 사용자를 선택하세요")
				...
				await RespondAsync($"'{username}' 이름을 가진 사용자가 여러 명 있습니다. 역할을 바꿀 사용자를 선택하세요:", components: builder.Build(), ephemeral: true);
		...
		// 역할 변경 확인 버튼 클릭 시 실행될 메서드
		[ComponentInteraction("change_role_confirm_button:*:*")]
		public async Task ChangeRoleConfirmButton(string userId, string roleName)
		{
			...
			try
			{
				...
				{
					await FollowupAsync($"❌ 역할 변경 실패: {result.ErrorMessage}", ephemeral: true);
					Logger.Print($"역할 변경 실패: {result.ErrorMessage}", LogType.ERROR);
				}
			}
			catch (Exception ex)
			{
				await FollowupAsync("역할 변경 중 오류가 발생했습니다.", ephemeral: true);
				Logger.Print($"역할 변경 중 오류 발생: {ex.Message}", LogType.ERROR);
			}
		}

		// 사용자 선택 메뉴에서 사용자를 선택했을 때 실행될 메서드
		...
		public async Task UserSelectionMenu(string roleName, string[] selectedValues)
		{
			...
			try
			{
				...
				// 확인 버튼 표시
				var builder = new ComponentBuilder()
					.WithButton("역할 변경", $"change_role_confirm_button:{selectedUserId}:{roleName}", ButtonStyle.Danger);

				await FollowupAsync($"'{selectedUser.DisplayName}({selectedUser.Username}, {selectedUser.Id})' 사용자의 역할을 '{roleName}'으로 변경하시겠습니까?", components: builder.Build(), ephemeral: true);
