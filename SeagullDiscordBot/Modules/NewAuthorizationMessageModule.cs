...
namespace SeagullDiscordBot.Modules
{
	...
	public partial class NewAuthorizationMessageModule : InteractionModuleBase<SocketInteractionContext>
	{
		...
		public async Task ChangeAuthorizationMessageOnCommand()
		{
			...
			// 로그 남기기
			Logger.Print($"서버 '{Context.Guild.Name}'({Context.Guild.Id})에서 '{Context.User.Username}'님이 change_authorization_message 명령어를 사용했습니다.");
		}
		...
		{
			Logger.Print($"서버 '{Context.Guild.Name}'({Context.Guild.Id})에서 '{Context.User.Username}'님이 사용자 인증 채널 안내 메시지 변경 버튼을 클릭했습니다.");
			// 모달 생성
		...
		public async Task MakeNewAuthorizationMessageModal(NewAuthMessageModal modal)
		{
			...
			try
			{
				...
				if (settings.AuthChannelId == null)
				{
					...
					Logger.Print($"서버 '{Context.Guild.Name}'({Context.Guild.Id})에서 '{Context.User.Username}'님이 인증 메시지 변경을 시도했지만 인증 채널이 설정되지 않았습니다.", LogType.WARNING);
					return;
				}

				// 인증 채널 가져오기
				var authChannel = Context.Guild.GetChannel(settings.AuthChannelId.Value) as ITextChannel;
				if (authChannel == null)
				{
					await FollowupAsync("인증 채널을 찾을 수 없습니다. 채널이 삭제되었거나 봇이 접근할 수 없습니다.", ephemeral: true);
					Logger.Print($"서버 '{Context.Guild.Name}'({Context.Guild.Id})에서 '{Context.User.Username}'님이 인증 메시지 변경을 시도했지만 인증 채널(ID: {settings.AuthChannelId})을 찾을 수 없습니다.", LogType.ERROR);
					return;
				}
				...
				// 로그 남기기
				Logger.Print($"서버 '{Context.Guild.Name}'({Context.Guild.Id})에서 '{Context.User.Username}'님이 인증 채널의 안내 메시지를 변경했습니다.");
			}
			catch (Exception ex)
			{
				Logger.Print($"서버 {Context.Guild.Id} 인증 메시지 변경 중 오류 발생: {ex.Message}", LogType.ERROR);
